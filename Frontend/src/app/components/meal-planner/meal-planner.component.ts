...
export class MealPlannerComponent implements OnInit {
  ...
  regenerateMealPlan() {
    ...
    this.aiService.generateMealPlan().subscribe({
      next: (plan: any) => {
        this.snackBar.open('🐉 Shenron is preparing your new meal plan!', 'Close', { duration: 2000, panelClass: ['dbz-snackbar'] });
        // Reload meals to get the updated data from server
        this.loadMeals();
      },
      error: (err: any) => {
        this.snackBar.open('💀 Shenron could not grant your meal wish!', 'Close', { duration: 3000, panelClass: ['dbz-snackbar'] });
        this.loading = false;
      }
    });
  }
